...
interface IIggyPostNftMetadataV2 {
  function getMetadata(uint256 _tokenId, string memory _postId, address _author, string memory _textPreview, string memory _image, uint256 _timestamp) external view returns (string memory);
}

/// @title Iggy Social Post NFT (v2)
contract IggyPostNft1155V2 is ERC1155, Ownable, ReentrancyGuard {
  ...
  struct Post {
    ...
    address author;
    string textPreview; // optional, can be empty
    string image; // optional, if there's image in the post
    uint256 timestamp;
  }
  ...
  function uri(uint256 _tokenId) public view override returns (string memory) {
    ...
    return IIggyPostNftMetadataV2(metadataAddress).getMetadata(
      _tokenId, post.postId, post.author, post.textPreview, post.image, post.timestamp
    );
  }
