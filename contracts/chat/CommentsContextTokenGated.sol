...
/**
 * @title CommentsContextTokenGated
 * @author TempeTechie.eth
 * @notice This contract is used to store comments within a certain context. Context can be comments under an NFT, playlist, etc. (whatever has an address and is an NFT or fungible token). Only users with a certain balance of a token are allowed to post.
 * 
 * Features:
 * - Simple comments thread under a crypto subject such as an NFT, a playlist, a blog post, etc. (it needs to have an address and be an NFT or fungible token)
 * - Only users with a certain balance of a token are allowed to post
 ...
 */
contract CommentsContextTokenGated is Ownable {
  uint256 public commentMinBalance; // minimum balance of token required to be able to post a comment (e.g. 1 NFT, 100 ERC-20 tokens, etc.)
  ...
  mapping(address => Comment[]) public comments; // mapping from subject address (NFT or fungible token) to array of comments
  ...
  // CONSTRUCTOR
  constructor(
    address modTokenAddress_, 
    address owner_, 
    uint256 commentMinBalance_,
    uint256 modMinBalance_,
    uint256 price_
  ) {
    modTokenAddress = modTokenAddress_;
    modMinBalance = modMinBalance_;
    commentMinBalance = commentMinBalance_;
    price = price_;
  ...
  modifier onlyMods() {
    require(
      IToken(modTokenAddress).balanceOf(msg.sender) >= modMinBalance || 
      msg.sender == owner(),
      "Not a mod or owner"
    );
    _;
  ...
  function isUserMod(address user_) public view returns (bool) {  
    return IToken(modTokenAddress).balanceOf(user_) >= modMinBalance || user_ == owner();
  }
   ...
   * @param subjectAddress_ The address of the comments subject (NFT or fungible token) to comment to
  ...
  function createComment(address subjectAddress_, string memory url_) external payable {
    ...
    require(IToken(subjectAddress_).balanceOf(msg.sender) >= commentMinBalance, "You do not have the minimum balance to post a comment");
