...
/** 
@title Default metadata contract for ERC-721 NFTs with media support
@author Tempe Techie
*/
contract NftMetadataMedia {
  string public constant mdContractType = "media";
  ...
  function getCollectionAudioUrl(address nftAddress_) external view returns (string memory) {
    return mds[nftAddress_].audioUrl;
  }

  function getCollectionAnimationUrl(address nftAddress_) external view returns (string memory) {
    return mds[nftAddress_].animationUrl;
  }
  ...
  function getCollectionYoutubeUrl(address nftAddress_) external view returns (string memory) {
    return mds[nftAddress_].youtubeUrl;
  }
  ...
  function getMetadata(address nftAddress_, uint256 tokenId_) external view returns (string memory) {
    ...
    return string(
      abi.encodePacked("data:application/json;base64,", Base64.encode(bytes(abi.encodePacked(
        ...
        _addUrls(nftAddress_),
        '"description": "', mds[nftAddress_].description, '"',
        '}'))))
    );
  }

  function _addUrls(address nftAddress_) internal view returns (string memory) {
    return string(abi.encodePacked(
      '"animation_url": "', mds[nftAddress_].animationUrl, '", ',
      '"audio_url": "', mds[nftAddress_].audioUrl, '", ',
      '"youtube_url": "', mds[nftAddress_].youtubeUrl, '", '
    ));
  }
  ...
  function setAnimationUrl(address nftAddress_, string memory animationUrl_) external {
    require(msg.sender == INFT(nftAddress_).owner(), "Not owner of NFT smart contract");
    mds[nftAddress_].animationUrl = animationUrl_;
  }

  function setAudioUrl(address nftAddress_, string memory audioUrl_) external {
    require(msg.sender == INFT(nftAddress_).owner(), "Not owner of NFT smart contract");
    mds[nftAddress_].audioUrl = audioUrl_;
  }
  ...
  function setYoutubeUrl(address nftAddress_, string memory youtubeUrl_) external {
    require(msg.sender == INFT(nftAddress_).owner(), "Not owner of NFT smart contract");
    mds[nftAddress_].youtubeUrl = youtubeUrl_;
  }
